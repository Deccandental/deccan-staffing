...
function TempsPageBody() {
  ...
  const [confirmConvertToStaff, setConfirmConvertToStaff] = useState<string | null>(null);
  const [convertingToStaff, setConvertingToStaff] = useState(false);
  ...
  async function handleConvertToEmployee(temp: TempStaff) {
    setConvertingToStaff(true);
    const validRoles: EmployeeRole[] = ["Dentist", "RDA", "Assistant", "Front Desk", "Hygienist"];
    const role: EmployeeRole = (validRoles as string[]).includes(temp.role) ? (temp.role as EmployeeRole) : "Assistant";
    const created = await addEmployee({
      name: temp.name,
      role,
      color: "#2563eb",
      skills: temp.skills.length > 0 ? temp.skills : [role],
      email: temp.email,
      pin: "",
      canAdmin: false, canManageLeave: false, canManageEvents: false, canManageCerts: false,
      archived: false,
      defaultSchedule: { monday: true, tuesday: false, wednesday: true, thursday: true, friday: true },
    });
    if (!created) {
      alert("Something went wrong converting this person to staff. They were not changed.");
      setConvertingToStaff(false);
      return;
    }
    await deleteTemp(temp.id);
    setConfirmConvertToStaff(null);
    setConvertingToStaff(false);
    await refresh();
  }
              ...
              return (
                <div key={temp.id} className="rounded-2xl bg-white shadow overflow-hidden">
                  ...
                  <div className="p-5">
                    ...
                    {confirmConvertToStaff === temp.id ? (
                      <div className="flex items-center justify-between rounded-lg bg-amber-50 border border-amber-100 px-3 py-2 mb-2">
                        <span className="text-xs text-amber-700">Convert to a full employee?</span>
                        <div className="flex gap-1 flex-shrink-0">
                          <button onClick={() => handleConvertToEmployee(temp)} disabled={convertingToStaff}
                            className="rounded-lg px-2 py-1 text-xs bg-amber-100 text-amber-700 hover:bg-amber-200 transition disabled:opacity-50">
                            {convertingToStaff ? "Converting…" : "Confirm"}
                          </button>
                          <button onClick={() => setConfirmConvertToStaff(null)}
                      ...
                      <button onClick={() => setConfirmConvertToStaff(temp.id)} className="text-xs text-slate-400 hover:text-slate-600 underline mb-2 inline-block">
                        Convert to employee →
                      </button>
                    )}
