...
import {
  StaffEvent, NewEventInput, loadUpcomingEvents, createEvent, deleteEvent,
...
function EventsPageBody() {
  ...
  async function handleCreate() {
    ...
    setSaving(true);
    const created = await createEvent({ ...form, title: form.title.trim(), description: form.description.trim() });
    if (created && announceNow) {
      try {
        ...
        {showForm && (
          <div className="mb-6 rounded-2xl bg-white p-6 shadow max-w-2xl">
            ...
            <label className="flex items-center gap-2 mb-5 cursor-pointer rounded-xl bg-orange-50 border border-orange-100 px-3 py-2.5">
              ...
              <span className="text-sm font-medium text-orange-700">📣 Also send an announcement email right now</span>
            </label>
              ...
              return (
                <div key={ev.id} className="rounded-2xl bg-white p-5 shadow flex items-start justify-between gap-4">
                  ...
                  <button onClick={() => handleDelete(ev.id)}
                    className="flex-shrink-0 rounded-lg px-3 py-1.5 text-xs text-red-400 hover:bg-red-50 hover:text-red-600 transition font-medium">
                    Delete
                  </button>
                </div>
              );
