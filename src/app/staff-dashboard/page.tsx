        ...
        {selectedId == null ? (
          ...
          <div className="grid gap-6 lg:grid-cols-2 max-w-5xl">
            ...
            {selectedEmployee?.hoBonusEligible && (
              <div className="lg:col-span-2 rounded-2xl bg-white p-5 shadow">
                <h2 className="font-bold text-slate-700 mb-3">💰 {bonusYear} Bonus (40% of Production, paid the following month)</h2>
                <div className="space-y-1.5">
                  {hoMonths.map((m) => {
                    const owed = m.production * 0.4;
                    const balance = owed - m.paid;
                    return (
                      <div key={`${m.year}-${m.month}`} className="flex items-center justify-between text-sm bg-slate-50 rounded-lg px-3 py-2">
                        <span className="font-medium text-slate-700">{new Date(m.year, m.month - 1, 1).toLocaleDateString("en-US", { month: "short", year: "numeric" })}</span>
                        <div className="flex items-center gap-3">
                          <span className="text-slate-400 text-xs">40%: ${formatMoney(owed)}</span>
                          {balance > 0 ? (
                            <span className="text-amber-600 font-semibold">Bonus: ${formatMoney(balance)}</span>
                          ) : m.production > 0 ? (
                            <span className="text-emerald-700 font-semibold">✓ Fully paid</span>
