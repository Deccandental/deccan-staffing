            ...
            {(adding || editing) && (
              <div className="rounded-2xl bg-white p-6 shadow space-y-5">
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    {DAYS.map((day, i) => (
                      <button key={day} onClick={() => toggleDay(day)} disabled={false} className="flex-1 rounded-lg py-2 text-xs font-semibold transition" style={form.defaultSchedule[day] ? { backgroundColor: "#e8622a", color: "white" } : { background: "#f1f5f9", color: "#9ca3af" }}>
                        {DAY_LABELS[i]}
                      </button>
                    ))}
                  </div>
