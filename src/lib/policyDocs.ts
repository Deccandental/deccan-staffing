...
export interface PolicyDocument {
  ...
  countersignerEmployeeId: number | null; // if set, only this employee (or Super Admin) can see/countersign a restricted document
}
...
function fromDocRow(row: any): PolicyDocument {
  return {
    id: row.id, slug: row.slug, title: row.title, cycleMode: row.cycle_mode,
    restrictedToEmployeeId: row.restricted_to_employee_id ?? null,
    countersignerEmployeeId: row.countersigner_employee_id ?? null,
  };
}
