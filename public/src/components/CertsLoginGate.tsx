...
const SESSION_KEY = "dd_certs_identity";

interface Props {
  children: (identity: CertsIdentity, logout: () => void) => ReactNode;
}

export default function CertsLoginGate({ children }: Props) {
  const [checked, setChecked] = useState(false);
  const [identity, setIdentity] = useState<CertsIdentity | null>(null);
  ...
  function persist(id: CertsIdentity) {
    setIdentity(id);
  ...
  function handleLogin() {
    ...
    const managerMatch = staff.find((e) => e.pin && e.pin === code && e.canManageCerts && !e.archived);
    if (managerMatch) {
      persist({ mode: "manager" });
      return;
    }
      ...
      <main className="min-h-screen" style={{ background: "#f5f5f5" }}>
        ...
        <div className="pt-16 lg:pt-0 lg:ml-64 flex items-center justify-center min-h-screen px-4">
          <div className="rounded-2xl bg-white p-6 sm:p-10 shadow-lg w-full max-w-sm text-center">
            ...
            <p className="text-gray-400 text-sm mb-8">Enter your personal PIN to upload or view your own certifications. Managers can enter their passcode instead.</p>
            <input
              type="password"
              inputMode="numeric"
